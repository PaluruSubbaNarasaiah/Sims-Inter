...
const AdminList = ({ admins, setAdmins, calculateRenewalDate, isPlanExpired, formatDate }) => {
  ...
  const [sortConfig, setSortConfig] = useState({ key: 'collegeName', direction: 'asc' });
  ...
  const handleSaveEdit = (id) => {
    ...
    // New: Check for unique contact number during edit
    ...
    setAdmins(prev =>
      prev.map(admin => {
        if (admin.id === id) {
          let newRenewalDate = admin.renewalDate;
          let newStatus = admin.status;
          let newManualOverride = admin.manualStatusOverride;
          let newProfileImage = editProfileImagePreview; // Use the preview URL

          if (editData.shouldRenewPlan) {
            newRenewalDate = calculateRenewalDate(editData.planType);
    ...
    setEditingId(null);
    setEditError('');
    setEditProfileImageFile(null); // Clear file input after save
  };
      ...
      prevAdmins.map(admin => {
        if (admin.id === id) {
          ...
          if (newStatus === true) { // If user is trying to set status to ACTIVE
            if (isPlanExpired(admin.renewalDate)) {
  ...
  const handleExport = () => {
    const headers = [
      "College Name", "Email ID", "User ID", "Contact Number", "Plan Type", // Added Contact Number
    ...
    const csvRows = filteredAndSortedAdmins.map(admin => { // Use filteredAndSortedAdmins for export
      ...
      return [
        `"${admin.collegeName.replace(/"/g, '""')}"`, // Handle quotes in college name
                  ...
                  <tr className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {editingId === admin.id ? (
                        ...
                        <div className="text-sm font-medium text-gray-900">{admin.collegeName}</div>
                      )}
                    </td>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="md:col-span-2 space-y-1">
                                ...
                                <div
                                  ...
                                  onClick={() => document.getElementById(`edit-profileImage-${admin.id}`).click()}
                                >
                                    <input
                                        type="file"
                                        id={`edit-profileImage-${admin.id}`}
                                        name="profileImage"
                                        accept="image/*"
