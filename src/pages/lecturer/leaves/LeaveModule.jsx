// lecturerLeaveModule.jsx (lecturer Panel)
...
import {
    ...
    Briefcase, // New icon for Leave Management button (lecturer's work-related leave)
} from 'lucide-react';
import LeaveManagement from './LeaveManagement'; // Import the new LeaveManagement component

// Mock data for leave requests relevant to a lecturer
const mockLeaveRequests = [
    ...
    },
    {
        ...
        endDate: '2024-08-01',
        status: 'Rejected',
        lecturerComment: 'Request rejected. Critical lecture on date specified.',
    ...
    },
    {
        ...
        status: 'Pending',
        lecturerComment: '',
        requestedAt: '2024-07-12T11:45:00Z',
    }
];
...
const LeaveModule = () => {
    ...
    const [activeView, setActiveView] = useState('studentLeave'); // 'studentLeave' or 'lecturerLeave'
    ...
    const handleUpdateStatus = (requestId, newStatus) => {
        ...
        setEditingActionId(null); // Reset editingActionId after action is taken
    };

    // Handlers for comment modal
    const openCommentModal = (request) => {
        setSelectedRequestForComment(request);
        setModalComment(request.lecturerComment || '');
            ...
            setLeaveRequests(prevRequests =>
                prevRequests.map(request =>
                    request.id === selectedRequestForComment.id
                        ? { ...request, lecturerComment: modalComment }
                        : request
                )
            );
            closeCommentModal();
        ...
        <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4">
                ...
                <div className="flex gap-4">
                    ...
                    <button
                        onClick={() => setActiveView('lecturerLeave')}
                        className={`px-5 py-2.5 rounded-lg font-semibold flex items-center transition-colors
                            ${activeView === 'lecturerLeave' ? 'bg-purple-600 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                    >My Leave
                    </button>
                </div>
            </div>
                                        ...
                                        {leaveRequests.map(request => (
                                            <tr key={request.id} className="hover:bg-gray-50 transition duration-150">
                                                <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-900 sm:px-6 sm:py-4 sm:text-sm">
                                                    {request.childName} ({request.class}-{request.section})
                                                </td>
                                                ...
                                                {/* lecturer's Comment Column Logic */}
                                                <td className="px-3 py-2 text-xs text-gray-600 sm:px-6 sm:py-4 sm:text-sm">
                                                    {request.status === 'Pending' || editingActionId === request.id ? ( // Show edit button if pending or if "Edit" in actions column is clicked
                                                        ...
                                                        >
                                                            ...
                                                            {request.lecturerComment ? 'Edit Comment' : 'Add Comment'}
                                                        </button>
                                                    ) : (
            ...
            {activeView === 'lecturerLeave' && <LeaveManagement />}
        </div>
    );
};
