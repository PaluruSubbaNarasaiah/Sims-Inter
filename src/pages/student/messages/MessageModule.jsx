...
function MessageModule() {
  ...
  const filteredMessages = useMemo(() => {
    return messages.filter(message => {
      // Filter by tab
      if (activeTab === 'inbox') {
        if (message.status === 'sent' && message.sender === 'student@college.edu') return false; // Exclude sent by current user
        if (message.status === 'draft' || message.status === 'trash') return false; // Exclude drafts and trash
      }

      if (activeTab === 'sent' && (message.status !== 'sent' || message.sender !== 'student@college.edu')) return false; // Only show sent by current user
  ...
  const unreadCount = useMemo(() => messages.filter(m => !m.read && m.status === 'sent' && m.sender !== 'student@college.edu' && m.status !== 'trash').length, [messages]);
