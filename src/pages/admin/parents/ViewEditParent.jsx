...
function ViewEditParent({ data, editable = false, onClose, onUpdate, existingParents = [] }) {
  ...
  useEffect(() => {
    if (data) {
      setFormData({
        ...data,
        childrenIds: data.childrenIds || [], // Ensure childrenIds is set from data or defaults to empty array
  ...
  const validateForm = () => {
    ...
    if (editable && !Array.isArray(childrenIds)) {
      newErrors.childrenIds = 'Student IDs must be a comma-separated list of IDs';
    } else if (editable && childrenIds.some(id => id.trim() === '')) {
      newErrors.childrenIds = 'Student IDs cannot be empty strings';
    }
      ...
      <div className="bg-white rounded-lg p-6 w-[90%] md:w-[450px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            />
            {errors.childrenIds && <p className="text-red-500 text-sm mt-1">{errors.childrenIds}</p>}
          </div>
