...
const ViewPayments = ({ onBackToFeeManagement }) => {
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    const mockPaymentsData = [
                        {
                            id: 1, invoiceId: 'INV-1720950000-A1B2C', studentId: 'MPC-001', studentName: 'Arjun Reddy', program: 'MPC', semester: 1,
                            term: '1st Term', amount: 5000, paymentDate: '2024-05-10', paymentMethod: 'UPI',
                            transactionId: 'TXN1234567890', status: 'Pending'
                        },
                        {
                            id: 2, invoiceId: 'INV-1720950100-D3E4F', studentId: 'CEC-002', studentName: 'Priya Sharma', program: 'CEC', semester: 1,
                            term: '1st Term', amount: 4500, paymentDate: '2024-05-12', paymentMethod: 'Bank Transfer',
                            transactionId: 'TXN0987654321', status: 'Pending'
                        },
                        {
                            id: 3, invoiceId: 'INV-1720950200-G5H6I', studentId: 'MPC-001', studentName: 'Arjun Reddy', program: 'MPC', semester: 1,
                            term: '2nd Term', amount: 3000, paymentDate: '2024-06-01', paymentMethod: 'Credit Card',
                            transactionId: 'TXN1122334455', status: 'Pending'
                        },
                        {
                            id: 4, invoiceId: 'INV-1720950300-J7K8L', studentId: 'CEC-002', studentName: 'Priya Sharma', program: 'CEC', semester: 1,
                            term: '2nd Term', amount: 1500, paymentDate: '2024-06-05', paymentMethod: 'UPI',
                            transactionId: 'TXN6677889900', status: 'Pending'
                        },
                        {
                            id: 5, invoiceId: 'INV-1720950400-M9N0O', studentId: 'HEC-003', studentName: 'Sneha Devi', program: 'HEC', semester: 2,
                            term: '1st Term', amount: 4000, paymentDate: '2024-05-15', paymentMethod: 'Bank Transfer',
                            transactionId: 'TXN1212121212', status: 'Pending'
                        },
                        {
                            id: 6, invoiceId: 'INV-1720950500-P1Q2R', studentId: 'BiPC-004', studentName: 'Rohan Joshi', program: 'BiPC', semester: 2,
                            term: '1st Term', amount: 3500, paymentDate: '2024-05-20', paymentMethod: 'UPI',
                            transactionId: 'TXN3434343434', status: 'Pending'
                        },
                    ];
                    setPayments(mockPaymentsData);

                    const programs = [...new Set(mockPaymentsData.map(p => p.program))].sort();
                    const semesters = [...new Set(mockPaymentsData.map(p => p.semester))].sort((a, b) => a - b);
    ...
    const filteredPayments = payments.filter(payment => {
        const matchesSearch = searchTerm === '' ||
            ...
            (payment.invoiceId && payment.invoiceId.toLowerCase().includes(searchTerm.toLowerCase())); // Include invoiceId in search

        return (
            matchesSearch &&
            (filters.program === '' || payment.program === filters.program) &&
            (filters.semester === '' || payment.semester === parseInt(filters.semester)) &&
    ...
    const handleExport = () => {
        const headers = [
            "Payment ID", "Invoice ID", "Student ID", "Student Name", "Program", "Semester", "Term", "Amount",
            "Payment Date", "Payment Method", "Transaction ID", "Status"
        ];
        const rows = filteredPayments.map(payment => [
            payment.id, payment.invoiceId, payment.studentId, payment.studentName, payment.program, payment.semester,
                                ...
                                {filteredPayments.length > 0 ? (
                                    filteredPayments.map((payment) => (
                                        <tr key={payment.id} className="hover:bg-gray-50 transition duration-150">
                                            <td className="px-3 py-2 whitespace-nowrap text-xs font-medium text-blue-600 sm:px-6 sm:py-4 sm:text-sm">
                                                {payment.invoiceId}
                                            </td>
                                            ...
                                            <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-600 sm:px-6 sm:py-4 sm:text-sm">
                                                {payment.program}/Year {payment.semester}
                                            </td>
                                            <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-600 sm:px-6 sm:py-4 sm:text-sm">
                                                {payment.term}
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan="12" className="px-6 py-8 text-center text-sm text-gray-500 sm:text-md">
