  ...
  const getClassReportData = useCallback((className) => {
    ...
    const sectionSummary = sections.map(section => {
      ...
      studentsInSection.forEach(student => {
        ...
        totalMarksAllExams += studentOverallMarks;
        totalMaxMarksAllExams += studentOverallMaxMarks;
      });
    ...
    <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b border-gray-200">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            onClick={() => { setActiveReportClass(null); setShowCompleteCollegeReport(true); }}
            className={`px-5 py-2.5 rounded-lg font-semibold transition-colors duration-200 shadow-md flex items-center justify-center ${showCompleteCollegeReport ? 'bg-indigo-600 text-white hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-500 focus:ring-opacity-50 transform hover:-translate-y-0.5' : 'bg-gray-200 text-gray-800 hover:bg-gray-300 focus:outline-none focus:ring-4 focus:ring-gray-300 focus:ring-opacity-50'}`}
          >
            <Users className="mr-2" size={20} /> College Report
          </button>
        </div>
      </div>

      {!showCompleteCollegeReport && (
        <>
          <h2 className="text-2xl font-bold text-gray-800 mb-6">Year Performance Overview</h2>
          
          <div className="mb-6">
            <label htmlFor="groupFilter" className="block text-sm font-medium text-gray-700 mb-2">Filter by Group:</label>
            <select
              id="groupFilter"
              className="block w-48 px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            >
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 border-b border-gray-200 mb-8 pb-3 -mx-4 px-4 sm:mx-0 sm:px-0 overflow-x-auto no-scrollbar">
            {allClasses.map(cls => (
              <button
                ...
                className={`flex-shrink-0 px-4 py-2 text-sm font-medium focus:outline-none transition-all duration-200 rounded-lg whitespace-nowrap ${activeReportClass === cls ? 'bg-blue-600 text-white shadow-md hover:bg-blue-700 transform hover:-translate-y-0.5' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                {cls}
              </button>
            ))}
          ...
          {activeReportClass && currentClassReport ? (
            currentClassReport.totalStudentsConsideredForOverall > 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mt-8">
                <div className="lg:col-span-3 bg-white p-6 rounded-xl shadow-md border border-gray-200">
                  ...
                  {currentClassReport.sectionSummary.length === 0 && (
                    <p className="text-center text-gray-500 py-6">No department data for this year (students might not have grades recorded).</p>
                  )}

                  {currentClassReport.subjectPerformance.length > 0 && (
                    <div>
                      <h4 className="text-lg font-semibold text-gray-700 mb-4 border-b pb-2">Subjects Performance (by Grade Category)</h4>
                      <div className="overflow-x-auto rounded-lg border border-gray-200">
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                  ...
                  {currentClassReport.subjectPerformance.length === 0 && (
                    <p className="text-center text-gray-500 py-6">No course performance data for this year (students might not have grades recorded).</p>
                  )}
                </div>
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-md border border-gray-200 flex flex-col justify-between">
                  <div>
                    ...
                    {currentClassReport.overallClassChartData.length > 0 && currentClassReport.totalStudentsConsideredForOverall > 0 ? (
                      ...
                      <div className="text-center text-gray-500 py-10">No overall performance data for this year yet. Make sure exams are 'Completed' and grades are in.</div>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="text-center text-gray-500 py-20 text-lg rounded-xl bg-white shadow-md p-8 border border-gray-200">
              Select a Year (e.g., Year 1) to view its detailed performance reports.
            </div>
          )}
        </>
      )}
      ...
      {showCompleteCollegeReport && (
        <div className="grid grid-cols-1 mt-8">
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
              <Users size={24} className="text-indigo-600" /> Complete College Performance Overview
            </h3>
            {completeSchoolReportData.totalStudentsConsidered > 0 && completeSchoolReportData.chartData.length > 0 ? (
              ...
              <div className="text-center text-gray-500 py-10">No overall college performance data available yet. Ensure exams are 'Completed' and grades are recorded across all years.</div>
            )}
          </div>
        </div>
      )}
