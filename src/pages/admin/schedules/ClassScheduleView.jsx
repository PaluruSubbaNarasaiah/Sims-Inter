...
const categorizeAndSortSchedules = (schedules, selectedProgram) => {
  const categories = {
    "Formative Assessment": [], // Will hold schedule objects for FA1, FA2, FA3 for the selected program
    "Summative Assessment": [] // Will hold schedule objects for SA1, SA2, SA3 for the selected program
  };

  // 1. Filter schedules relevant to the currently selected stream
  const filteredByProgram = schedules.filter(
    (schedule) => schedule.program === selectedProgram
  );

  // 2. Assign each filtered schedule to its main category
  filteredByProgram.forEach(schedule => {
...
const ClassScheduleView = ({ allSchedules, selectedProgram, onEditSchedule, onDeleteSchedule }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const categories = categorizeAndSortSchedules(allSchedules, selectedProgram);
  ...
  }, [allSchedules, selectedProgram]); // Re-run this effect when allSchedules data or selectedProgram changes
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-64 text-gray-700">
        ...
        <p className="mt-3">Loading schedules for {selectedProgram}...</p>
      </div>
    );
  }
  ...
  // Determine if there are any schedules at all for the selected stream across all categories
  const hasAnySchedules = categorizedSchedules && Object.values(categorizedSchedules).some(arr => arr.length > 0);

  return (
    <div className="space-y-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">{selectedProgram} Exam Schedule</h2>

      {/* Message displayed if no schedules exist for the current stream in any category */}
      {!hasAnySchedules && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 p-6 rounded-lg shadow-md" role="alert">
          ...
          <p>There are no exam schedules for <span className="font-semibold">{selectedProgram}</span> stream across any assessment types yet. Click 'Create Schedule' above to add one.</p>
        </div>
      )}

      {/* Formative Assessment Section */}
        <h3 className="text-xl font-bold text-gray-800 mb-4">Formative Assessment</h3>
        {/* Check if there are any formative schedules for the current stream */}
        {categorizedSchedules["Formative Assessment"].length === 0 ? (
          <div className="bg-gray-50 border-l-4 border-gray-300 text-gray-700 p-4 rounded-md">
            <p>No available schedules for Formative Assessments in {selectedProgram} stream.</p>
          </div>
        ) : (
      ...
      {/* Summative Assessment Section */}
        ...
        {/* Check if there are any summative schedules for the current program */}
        {categorizedSchedules["Summative Assessment"].length === 0 ? (
          <div className="bg-gray-50 border-l-4 border-gray-300 text-gray-700 p-4 rounded-md">
            <p>No available schedules for Summative Assessments in {selectedProgram}.</p>
          </div>
        ) : (
          <div className="overflow-x-auto rounded-lg shadow-md">
            <table className="min-w-full divide-y divide-gray-200">
